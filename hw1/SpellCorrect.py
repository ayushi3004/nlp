...
class SpellCorrect:
  ...
  def correctSentence(self, sentence):
    ...
    for i in range(1, len(sentence) - 1): #ignore <s> and </s>
      corrected_word_list = self.editModel.editProbabilities(sentence[i])
      trialSentence = sentence[:]

      for corrected_word, corrected_prob in corrected_word_list:
        trialSentence[i] = corrected_word
        trial_score = self.languageModel.score(trialSentence)

        score = trial_score + corrected_prob
        if score >= bestScore:
          bestScore = score
