...
class EditModel(object):
  ...
  def insertEdits(self, word):
    ...
    #      signal is 't' and the correct signal is 'ta'. See slide 14 of the noisy channel model.
    ...
    for i in range(0, len(word)):
      for j in range(0, len(EditModel.ALPHABET)):
  ...
  def transposeEdits(self, word):
    ...
    #      and the correct signal is 'et'. See slide 14 of the noisy channel model.
    word = "<" + word  # append start token
    if len(word) <= 0:
      return []
    ...
    for i in range(1, len(word)-1):
      for j in range(2, len(word)):
        corruptLetters = word[i] + word[j]
        correctLetters = word[j] + word[i]
        correction = "%s%s%s" % (word[j], word[i], word[j+1:])
        ret.append(Edit(correction, corruptLetters, correctLetters))
    return ret
  ...
  def replaceEdits(self, word):
    ...
    #      and the correct signal is 'q'. See slide 14 of the noisy channel model.
    word = "<" + word  # append start token
    if len(word) <= 0:
      return []
    ...
    for i in range(1, len(word)):
      for j in range(0, len(EditModel.ALPHABET)):
        if EditModel.ALPHABET[j] != word[i]:
          corruptLetters = word[i]
